...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	meanSpeedMC := meanSpeed(action, duration) * kmhInMsec
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeedMC, 2)/height*cmInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
